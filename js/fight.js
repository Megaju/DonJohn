...
function generateMonster(mode) {
    if (mode === 1) {
        ...
        var eMonster = new Monster(
            ...
            0); //potion
        log(`life : ${eMonster.life} atk : ${eMonster.atk} def : ${eMonster.def} agi : ${eMonster.agi}`)
    } else if (mode === 2) {
        // normal mode
    ...
    // starting fight
    if (hero.agi >= eMonster.agi) {
        log(`${hero.name} commence le combat avec ${hero.agi} AGI contre ${eMonster.agi}.`);
        agiTest(eMonster, hero);
    } else {
        log(`Le monstre commence le combat avec ${eMonster.agi} AGI contre ${hero.agi}.`);
        agiTest(hero, eMonster);
    }
